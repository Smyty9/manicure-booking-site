# 💅 Маникюр Салон - Сайт записи

Современный веб-сайт для онлайн записи к мастеру маникюра с прайс-листом услуг, интерактивным календарем и системой бронирования.

## 🎨 Особенности

- **📋 Прайс-лист услуг** - полный список услуг с ценами
- **📅 Интерактивный календарь** - выбор удобной даты
- **⏰ Выбор времени** - доступные временные слоты
- **🎯 Форма записи** - сбор контактных данных клиента
- **📱 Адаптивный дизайн** - оптимизирован для мобильных устройств
- **🔗 Webhook интеграция** - отправка данных на n8n
- **💾 Локальное хранение** - сохранение записей в браузере
- **🎨 Современный UI** - красивый и интуитивный интерфейс

## 📱 Функциональность

### Прайс-лист услуг
- **Маникюр:** от 1500 ₽ до 4000 ₽
  - Классический маникюр - 1500 ₽
  - Маникюр с гель-лаком - 2500 ₽
  - Маникюр с укреплением - 3000 ₽
  - Наращивание ногтей - 4000 ₽

- **Педикюр:** от 2000 ₽ до 3500 ₽
  - Классический педикюр - 2000 ₽
  - Педикюр с покрытием - 3000 ₽
  - SPA педикюр - 3500 ₽

- **Дополнительные услуги:** от 300 ₽ до 2500 ₽
  - Дизайн ногтей - 500 ₽
  - Снятие покрытия - 300 ₽
  - Коррекция нарощенных ногтей - 2500 ₽

### Календарь записи
- Навигация по месяцам
- Выделение текущего дня
- Блокировка прошедших дат
- Рабочие дни: понедельник - суббота
- Визуальная индикация доступных дат

### Выбор времени
- Временные слоты каждый час с 9:00 до 19:00
- Отображение занятых слотов
- Интуитивный выбор времени

### Форма записи
- Поле для имени (обязательное)
- Поле для телефона (обязательное)
- Поле для Telegram (опциональное)
- Выбор услуги из прайс-листа
- Сводка выбранной даты, времени и услуги
- Валидация полей

### Отправка данных
- Автоматическая отправка на webhook
- JSON формат данных
- Обработка ошибок
- Уведомление об успешной записи

## 🚀 Установка и запуск

### 1. Скачайте файлы проекта
```
index.html
style.css
script.js
```

### 2. Откройте index.html в браузере
- Двойной клик на файл или
- Перетащите файл в окно браузера

### 3. Для локального сервера (рекомендуется)
```bash
# Python 3
python -m http.server 8000

# Python 2
python -m SimpleHTTPServer 8000

# Node.js (если установлен)
npx http-server
```

### 4. Откройте в браузере
```
http://localhost:8000
```

## ⚙️ Настройка

### Изменение webhook URL
В файле `script.js` найдите строку:
```javascript
const WEBHOOK_URL = 'https://your-domain.beget.com/webhook-test/c31eacd5-e2d4-4bbd-b62f-647e52ebc493';
```

И замените на ваш URL n8n:
```javascript
const WEBHOOK_URL = 'https://ваш-домен.beget.com/webhook-test/c31eacd5-e2d4-4bbd-b62f-647e52ebc493';
```

### Изменение рабочих часов
В файле `script.js` настройте:
```javascript
// Рабочие дни (1 = понедельник, 0 = воскресенье)
const workingDays = [1, 2, 3, 4, 5, 6];

// Временные слоты
const availableTimeSlots = [
    '09:00', '10:00', '11:00', '12:00', '13:00', 
    '14:00', '15:00', '16:00', '17:00', '18:00', '19:00'
];
```

### Изменение цен и услуг
В файле `script.js` обновите:
```javascript
const servicePrices = {
    'classic-manicure': 'Классический маникюр - 1500 ₽',
    'gel-manicure': 'Маникюр с гель-лаком - 2500 ₽',
    // Добавьте или измените услуги
};
```

Также обновите HTML в `index.html` для отображения новых услуг.

### Изменение дизайна
Основные цвета и стили находятся в файле `style.css`:
- Основной цвет: `#e53e3e` (красный)
- Градиент фона: `linear-gradient(135deg, #667eea 0%, #764ba2 100%)`
- Шрифт: `Inter` (Google Fonts)

## 📊 Формат данных

При отправке на webhook передается JSON объект:
```json
{
  "date": "2024-12-20",
  "time": "14:00",
  "service": "gel-manicure",
  "serviceName": "Маникюр с гель-лаком - 2500 ₽",
  "clientName": "Анна Петрова",
  "clientPhone": "+7 (999) 123-45-67",
  "clientTelegram": "@anna_pet",
  "timestamp": "2024-12-19T10:30:00.000Z"
}
```

## 🔧 Технические детали

### Технологии
- **HTML5** - структура страницы
- **CSS3** - стили и анимации
- **JavaScript (ES6+)** - интерактивность
- **LocalStorage** - локальное хранение данных
- **Fetch API** - отправка данных на сервер

### Совместимость
- Chrome 60+
- Firefox 55+
- Safari 12+
- Edge 79+
- Мобильные браузеры

### Производительность
- Минимум зависимостей
- Оптимизированные анимации
- Быстрая загрузка
- Эффективное использование памяти

## 🌐 Размещение на хостинге

### Netlify (рекомендуется)
1. Перейдите на [netlify.com](https://netlify.com)
2. Зарегистрируйтесь
3. Перетащите папку с файлами в область загрузки
4. Получите URL типа: `https://amazing-name-123456.netlify.app`

### GitHub Pages
1. Создайте репозиторий на GitHub
2. Загрузите файлы
3. Включите GitHub Pages в настройках
4. Получите URL: `https://username.github.io/repository-name`

### Другие хостинги
- Vercel
- Firebase Hosting
- AWS S3
- Любой статический хостинг

## 🐛 Отладка

### Проверка консоли
Откройте Developer Tools (F12) и проверьте вкладку Console для ошибок.

### Проверка webhook
Используйте сервисы типа:
- [webhook.site](https://webhook.site)
- [ngrok](https://ngrok.com)
- [Postman](https://postman.com)

### Тестовые данные
Для демонстрации раскомментируйте строку в `script.js`:
```javascript
addSampleBookings();
```

## 📱 Мобильная оптимизация

Сайт полностью адаптирован для мобильных устройств:
- Адаптивная сетка услуг
- Touch-friendly интерфейс
- Оптимизированные размеры кнопок
- Правильные viewport настройки
- Улучшенная читаемость на маленьких экранах

## 🔒 Безопасность

### Рекомендации
- Используйте HTTPS для продакшена
- Валидируйте данные на сервере
- Ограничьте частоту запросов
- Настройте CORS правильно

### CORS настройки
Если возникают проблемы с CORS, добавьте в n8n workflow:
```json
{
  "operation": "set",
  "values": {
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "POST, GET, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type"
  }
}
```

## 📞 Поддержка

При возникновении вопросов или проблем:
1. Проверьте консоль браузера на ошибки
2. Убедитесь в правильности webhook URL
3. Проверьте подключение к интернету
4. Убедитесь в поддержке CORS на сервере

## 📝 Лицензия

Этот проект создан для демонстрационных целей. Используйте свободно для своих проектов.

## 🤝 Вклад в проект

Если вы хотите улучшить проект:
1. Создайте fork репозитория
2. Внесите изменения
3. Создайте pull request
4. Опишите изменения

## 📚 Ресурсы

- [n8n Documentation](https://docs.n8n.io/)
- [Netlify Documentation](https://docs.netlify.com/)
- [GitHub Pages](https://pages.github.com/)
- [MDN Web Docs](https://developer.mozilla.org/)

---

**Создано с ❤️ для удобной записи к мастеру маникюра**

**🎯 Идеально подходит для:** салонов красоты, маникюрных салонов, мастеров маникюра, beauty-бизнеса 